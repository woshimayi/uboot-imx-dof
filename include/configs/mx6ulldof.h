 ...
 */
#ifndef __MX6ULLDOF_CONFIG_H
#define __MX6ULLDOF_CONFIG_H
...
/*
 * NAND 分区布局 (i.MX6ULL DOF, 512MiB NAND, erase block 128KiB)
 *
 * env_a / env_b 各扩到 512KiB (4 个 erase block):
 *   - env/nand.c writeenv 循环会跳过坏块, 在 CONFIG_ENV_RANGE 范围内重试
 *   - 单 block 的 env 一旦该 block 变坏, saveenv 会直接 FAILED
 *   - 4 块 = 3 块坏块容差, 工程经验上够长生命周期
 * 必须与 defconfig 的 CONFIG_ENV_OFFSET / CONFIG_ENV_OFFSET_REDUND 保持一致:
 *   CONFIG_ENV_OFFSET        = 0x07000000   (env_a 起始)
 *   CONFIG_ENV_OFFSET_REDUND = 0x07080000   (env_b 起始 = env_a + 512KiB)
 */
#define MFG_NAND_MTDPARTS "gpmi-nand:8m(uboot_a),8m(uboot_b),32m(kfd_a),32m(kfd_b),16m(tee_a),16m(tee_b),512k(env_a),512k(env_b),175m(rootfs_a),175m(rootfs_b),-(date)"
...
/*
 * env 写入坏块容差: env/nand.c writeenv() 会在 [OFFSET, OFFSET+RANGE) 内
 * 跳过坏块继续找下一块。RANGE 必须 <= env_a/env_b 分区大小, 否则会越界
 * 写到 rootfs 区。当前 env_a/env_b 各 512KiB = 4 个 erase block。
 */
#if defined(CONFIG_ENV_IS_IN_NAND) && !defined(CONFIG_ENV_RANGE)
